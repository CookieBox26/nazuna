...
from nazuna.utils import as_path_if_length_safe, measure_time
...
class Workflow:
    ...
    def __post_init__(self):
        ...
        for i_task, _ in enumerate(self.tasks):
            ...
            self.tasks[i_task].setdefault(
                'out_dir', (self.out_path / Workflow._to_snake(self.tasks[i_task]['name'])).as_posix(),
            )
    ...
    def run_tasks(self, skip_task_ids_: str = ''):
...
def run_tasks(
    ...
    Workflow.create(conf_).run_tasks(skip_task_ids_=skip_task_ids_)
