...
class _TwoStageAttentionLayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        dim_enc = self.norm_2(dim_enc)
        dim_enc = dim_enc + self.dropout(self.mlp(dim_enc))
        dim_enc = self.norm_3(dim_enc)
...
class _DecoderLayer(torch.nn.Module):
    def __init__(
        self, seg_len, out_seg_num, factor, d_model, n_heads, d_ff,
        dropout_aw, dropout_sa, dropout_ff,
    ):
        super().__init__()
        ...
        self.linear_pred = torch.nn.Linear(d_model, seg_len)
    ...
    def forward(self, x, cross):
        ...
        pred = self.linear_pred(x).reshape(B, C, S, -1)  # (B, C, out_seg_num, seg_len)
        return dec, pred


class _Decoder(torch.nn.Module):
    def __init__(
        self, n_layers, seg_len, pred_len, out_seg_num, factor,
        ...
        self.layers = torch.nn.ModuleList([
            _DecoderLayer(
                seg_len, out_seg_num, factor, d_model, n_heads, d_ff,
                dropout_aw, dropout_sa, dropout_ff,
            )
    ...
    def forward(self, x, cross_list):
        ...
        B, C, S, sl = final.shape  # (B, C, out_seg_num, seg_len)
        final = final.permute(0, 2, 3, 1).reshape(B, S * sl, C)
...
class Crossformer(BasicBaseModel):
    ...
    def _setup(
        ...
        seg_len: int = 12, win_size: int = 4, factor: int = 10,
        ...
        self.patchifier = Patchifier(seg_len, seg_len, 'end')
        self.in_seg_num = self.patchifier.num_patches(seq_len)
        self.out_seg_num = ceil(pred_len / seg_len)

        self.value_embed = torch.nn.Linear(seg_len, d_model)
        ...
        self.decoder = _Decoder(
            e_layers + 1, seg_len, pred_len, self.out_seg_num, factor,
            d_model, n_heads, d_ff, dropout_aw, dropout_sa, dropout_ff,
        )

    def forward(self, x):
        B, L, C = x.shape
        patches = self.patchifier(x)  # (B, C, in_seg_num, seg_len)
